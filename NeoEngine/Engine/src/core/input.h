...
#ifndef NEOENGINE_INPUT_H
#define NEOENGINE_INPUT_H
...
namespace NeoEngine {
    ...
    public:
        ...
        //mouse
        bool IsButtonDown(Keys key);
        bool IsButtonUp(Keys key);
        bool WasButtonDown(Keys key);
        bool WasButtonUp(Keys key);

        void GetMousePosition(int32_t* x, int32_t* y) const;
        void GetPreMousePosition(int32_t* x, int32_t* y) const;

        void ProcessButton(MouseButtons button, bool pressed);
        void ProcessMouseMove(int16_t x, int16_t y);
        void ProcessMouseWheel(int8_t z_delta);
    ...
    private:
        ...
        struct MouseState {
            int16_t x_, y_;
            uint8_t buttons_[static_cast<uint8_t>(MouseButtons::MOUSE_BUTTON_MAX)];
        };

        struct InputState {
            KeyboardState keyboard_current_;
            KeyboardState keyboard_previous_;
            MouseState mouse_current_;
            MouseState mouse_previous_;
        };
